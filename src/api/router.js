...
console.log(
	`Redirecting to ${
		dev ? "http://localhost:" + PORT + REDIRECT_URI : REDIRECT_URI
	}`
);
...
const handleError = (res, err) => {
	console.log("Got error! Error Name: %s", err.message);
...
router.get("/login", (req, res) => {
	const scopes =
		"user-read-private user-read-email user-library-read streaming user-top-read";
	res.redirect(
		"https://accounts.spotify.com/authorize" +
			...
			encodeURIComponent(
				`${
					dev ? "http://localhost:" + PORT : REDIRECT_URI
				}/api/authToAccess`
			)
	);
});
...
router.get("/authToAccess", async (req, res) => {
	...
	else if (!code) return res.redirect("/login");
	try {
		...
		const token = await jwt.sign({ access_token }, JWT_SECRET, {
			expiresIn: "1h",
		});
...
router.get("/album", async (req, res) => {
	...
	try {
		let resp = await makeSpotifyRequest(
...
router.get("/artist", async (req, res) => {
	...
	try {
		let resp = await makeSpotifyRequest(
			`https://api.spotify.com/v1/artists/${id}?country=SE`,
			token
		);
		let artist = resp.data;
		let albums = await makeSpotifyRequest(
			`https://api.spotify.com/v1/artists/${id}/albums?country=SE`,
			token
		);
		artist.albums = albums.data.items.filter(
			(thing, index, self) =>
				index === self.findIndex((t) => t.name === thing.name)
		);
		let topTracks = await makeSpotifyRequest(
			`https://api.spotify.com/v1/artists/${id}/top-tracks?country=from_token`,
			token
		);
		artist.topTracks = topTracks.data.tracks.map((obj, i) => ({
			...obj,
			track_number: i + 1,
		}));
		res.send(artist);
...
router.get("/saved", async (req, res) => {
	...
	try {
		let resp = await makeSpotifyRequest(
			`https://api.spotify.com/v1/me/${type}?limit=32&offset=${offset}`,
			token
		);
		res.send(resp.data);
	} catch (err) {
...
router.get("/track/:trackId", async (req, res) => {
	const { token } = req.cookies;
	const trackId = req.params.trackId || undefined;
...
router.get("/playlist", async (req, res) => {
	...
	try {
		if (all) {
			...
			resp = await makeSpotifyRequest(
				`https://api.spotify.com/v1/playlists/${id}/tracks?limit=32&offset=${offset}`,
				token
			);
